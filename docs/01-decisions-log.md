...
- **Note on the "free" variant (no OHLC vendor):** Documented in [research/ndw-data-link-alternatives.md Option A0a](research/ndw-data-link-alternatives.md#a0a-existing-dwa-platform-subscription--manual-csv-export-no-additional-ohlc-vendor--the-free-path) but **not recommended.** Without OHLC the bot loses ~70% of its analytical capability: no chart rendering, only ~2 of 7 pre-momentum patterns detectable, no backtesting, no own-engine validation. The bot effectively degenerates into a filtered view of the DWA export. The $53/month Norgate cost is the difference between an analytical screener and an Excel view — the cheapest thing about the project.
- **Status:** Pending advisor verification — he needs to confirm 4 things in his platform login: (1) can define a universe-wide screener with all relevant fields, (2) can export it as a single CSV, (3) BPI views are exportable, (4) total daily export effort is < 10 minutes. Also worth checking: whether DWA exports include raw OHLC columns (if yes, the free variant becomes more viable).
